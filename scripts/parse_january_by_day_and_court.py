...
async def main():
    ...
    async with PlaywrightScraper(use_cdp=True, cdp_url="http://localhost:9222") as scraper:
        ...
        START_DAY = 19  # Измените на 2 для полного парсинга

        print(f"📊 План: {32 - START_DAY} дней (с {START_DAY} по 31 января) × {len(courts)} судов = {(32 - START_DAY) * len(courts)} запросов\n")
        ...
        for day_num in range(START_DAY, 32):  # С START_DAY по 31 января
