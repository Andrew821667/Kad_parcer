...
Стратегия: ДЕНЬ + СУД (30 дней × ~100 судов = ~3000 запросов)
Ожидаемый результат: ~40,000 дел
"""
...
async def main():
    ...
    async with PlaywrightScraper(use_cdp=True, cdp_url="http://localhost:9222") as scraper:
        ...
        print(f"📊 План: {31 - START_DAY} дней (с {START_DAY} по 30 ноября) × {len(courts)} судов = {(31 - START_DAY) * len(courts)} запросов\n")
